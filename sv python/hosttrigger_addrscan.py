...
tableid=10
value='AddrScan' # Connection classifier Value
...
class Trigger(HostTrigger):
    def trigger(self):
        ...
        if (json_obj['status'] != "ok"):
            ...
            send_trap("Address scan threat detected on %s" % (self.distributed_field_list))
    ...
    def reset(self):
        ...
        if (json_obj['status'] != "ok"):
            ...
            send_trap("Address scan clear on %s" % (self.distributed_field_list))
